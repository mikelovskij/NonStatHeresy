...
from functions import brms_reader, ipsh, get_channel_list, even_less_simple_cumulative_psd_csd_correlation
import os
import mpld3
import fnmatch
...
hdir = '/users/valentin/PycharmProjects/Nonstatmoni_reader/Results/{}/'.format(brmsfile.split('/')[-2])
all_channels = True
try:
    os.mkdir(hdir)
except OSError:
...
class Parameters:
    def __init__(self, initfile):
        ...
        for l in xrange(self.nchannels):
            ...
            self.coherences.append(config.get(section, 'coherences').split('\n'))
...
gpsb, gpse, fsample, segments, times, brms_data = brms_reader(brmsfile, channelsandbands)
...
if all_channels:
    ...
    ipsh()
for channel, units, dt, nav, cohe, nchan in zip(par.channel, par.units, par.dt, par.nav,
                                                par.coherences, xrange(len(par.channel))):
    ...
    plt.legend(bands, loc=(1.02, 0.2))  # Todo: uso invece le bande in band_data, o almeno faccio un confronto per vedere se van bene?
    plt.title('BRMS %s - %s' % (gps2str(gpsb), gps2str(gpse)), fontsize=10)
    bmin = bands[0].split('_')[0]
    bmax = bands[-1].split('_')[1].split('Hz')[0]
    plt.savefig(hdir + pdir + channel + '_' + str(nchan) + '_' + dt + 's_' + bmin + '_' + bmax + 'Hz_time.png')
    plt.close()
    # ####### Compute and plot spectra
    N = len(band_data.values()[0])  # all bands should have the same length, according to 9 out of 10 statisticians
    ...
    for j in xrange(len(segments) - 1):
        if (segments[j][1] == segments[j + 1][0]) and ((segments[j + 1][1] - segments[j][0]) < max_length):
    ...
    for y in band_data.itervalues():
        if not data_storage:
            s1, s2, csds, mean_1, square_mean_1, aux_means, square_aux_means, prod_means, f, hist, data_storage = even_less_simple_cumulative_psd_csd_correlation(
                y - np.mean(y), times, cohe, NFFT, segments, fsample, source, nbins=150)
        else:
            s1, s2, csds, mean_1, square_mean_1, aux_means, square_aux_means, prod_means, f, hist, _ = less_simple_cumulative_psd_csd_correlation(
                y - np.mean(y), times, cohe, NFFT, segments, fsample, source, nbins=150, data_storage=data_storage)
        sp.append(s1)
        for psd, csd, aux_mean, square_aux_mean, prod_mean, h, k in zip(s2, csds, aux_means, square_aux_means,
                                                                        prod_means, hist, xrange(len(s2))):
            cohs[k].append((np.abs(csd)) ** 2 / (psd * s1))
            ccfs[k].append((prod_mean - mean_1 * aux_mean) / np.sqrt(
                (square_mean_1 - mean_1 ** 2) * (square_aux_mean - aux_mean ** 2)))
    ...
    for s in sp:
        try:
            ...
            print "Issue in plotting psd of channel {0} {1}, may there be no positive data?".format(channel, s)
    plt.axis(xmax=0.5)
    ...
    plt.title('BRMS %s - %s' % (gps2str(gpsb), gps2str(gpse)), fontsize=10)
    plt.savefig(hdir + pdir + channel + '_' + str(nchan) + '_' + dt + 's_' + bmin + '_' + bmax + 'Hz_psd.png')
    plt.close()

    for C, corrcoef, hist, cname, naux in zip(cohs, ccfs, histograms, cohe, xrange(len(cohs))):
        # C = []
        nplot = 0
        hicorrlist = []
        hicorrbands = []
        ...
        for b, j in zip(band_data.iteritems(), xrange(len(band_data.items()))):
            meancoh[naux].append(np.mean(C[j]))  # TODO: Isn't the maximum coherence more interesting?
        ...
        if nplot > 0:
            ...
            for j in xrange(n):
                for (k, h, b) in zip(xrange(m), hicorrlist[m * j:m * (j + 1)], hicorrbands[m * j:m * (j + 1)]):
        ...
        for c in C:
            try:
                ...
                print "Issue in plotting psd of channel {0} {1}, may there be no positive data?".format(cname, channel)
...
for c, num in zip(ch, chnum):
    ...
    for channel, dt, n in zip(par.channel[prenum: prenum + num],
        ...
        bmax = bands[-1].split('_')[1].split('Hz')[0]
        chbands = bmin + '_' + bmax
        page.add('<a href=#%s>%s</a><br>(dt=%s)' % (channel + chbands, chbands, dt))
        page.td.close()
    page.tr.close()
    prenum += num
page.table.close()
...
for (cname, naux) in zip(cohe, xrange(len(cohs))):
    ...
    for chan in ccfsort[0:4]:
        ...
        page.add(
            "<a target=_blank href={}{}>{}</a><br>(ccf={:3f})".format(hdir.replace(' ', '%20'), pagename,
    ...
    for chan in mncohsort[0:4]:
        ...
        page.add(
            '<a target=_blank href=%s/%s>%s</a><br>(mncoh=%3f)' % (hdir.replace(' ', '%20'),
                                                                   pagename, chbandsall[chan], meancoh[naux][chan]))
        page.td.close()
...
nbands = 0
for (channel, dt, cohe, nchan) in zip(par.channel, par.dt, par.coherences, xrange(len(par.channel))):
    bands = channelsandbands[channel + '_' + str(nchan)]
    bmin = bands[0].split('_')[0]
    bmax = bands[-1].split('_')[1].split('Hz')[0]
    ...
    for naux in xrange(len(cohs)):
        ...
        for (b, corr, coh, i) in zip(bands, ccf[naux][nbands:(nbands + len(bands))],
                                     meancoh[naux][nbands:(nbands + len(bands))], xrange(len(bands))):
            if corr > min(ccftab):
                tccftab = ccftab
                tidccftab = idccftab
    ...
    for i in xrange(ntop):
        row = ["{0}<br>Highest CCFs = {1}".format(cohe[idccftab[i]], ccftab[i]), "{0}<br>Highest Coher. = {1}".format(cohe[idcohtab[i]], cohtab[i])]
        tab.append(row)
    tabstr = tb.tabmaker(tab, True, False)

    page.div("<div style=\"float:left\"><img src=\"" + pdir + channel + '_' + str(
        nchan) + '_' + dt + "s_" + bmin + "_" + bmax + "Hz_time.png\" /></div><div>" + tabstr + "</div>")

    page.h2("Spectrum of BRMS time series")
    page.img(src=(pdir + channel + '_' + str(nchan) + '_' + dt + 's_' + bmin + '_' + bmax + 'Hz_psd.png'), alt="Plots")
    ...
    for cname in cohe:
        page2.img(src=(pdir + channel + '_' + str(nchan) + '_' + dt + 's_' + bmin + '_' + bmax + 'Hz_cohe_'
                       + cname + '.png'), alt="Plots")
    pagename = 'cohe_%s_%d_Hz.html' % (channel, nchan)
    # 'cohe_' + channel + '_' + dt + 's_' + bmin + '_' + bmax + 'Hz.html'
    page2.savehtml(hdir + pagename)
    page.h2(
        "<a target=_blank href=" + hdir.replace(' ', '%20') + pagename + ">Coherences with slow channels</a>")
    page.br()
